...
public class MiniShell {
			...
			public void verifyKey(VerifyEvent e) {
				...
				if (e.keyCode == SWT.ARROW_UP) {
					...
					if (currentCommand == -99) {
						...
						styledText.setText((String) commandhistory
								.get(commandhistory.size() - 1));
					} else {
						if (currentCommand > 0)
							currentCommand--;
						styledText.setText((String) commandhistory
	...
	private void doSearch() {
		if (lister == null || lister.isDisposed()) {
			fileManager.getActivePanel().incSearch(
					styledText.getText().substring(1), true, true);
		} else {
	...
	private void doCommand() {
		...
		if (styledText.getText().startsWith(":")) {
			...
			if (cmdText.indexOf(" ") > 0) {
				...
				args = replaceVariableValue(
						cmdText.substring(cmdText.indexOf(" ") + 1)).split(
						"(?<=[^\\\\\\\\])\\s+");
			} else {
				cmd = cmdText;
				args = null;
	...
	private void doAutoComplete() {
		...
		if (tokens.length == 1) {
			completeOptions = AutoCompleteUtil.getCommandCompleteList(tokens[0]
					.substring(1));
	...
	private void doCompletionFileList(String lastToken, String pwd, String cmd,
			String cmdText) {
		...
		String newCmdText = "";
		// absolute path
		if (lastToken.startsWith(File.separator)
				|| (lastToken.length() > 1 && lastToken.charAt(1) == ':')) {
			newCmdText = cmdText.substring(0, cmd.length() + 2) + filePath
					+ File.separator;
		} else { // relative path
			pwd = pwd.replaceAll(" ", "\\\\\\\\ ");
			...
			if (pwd.equals("/")) {
				newCmdText = cmdText.substring(0, cmd.length() + 2)
						+ filePath.substring(1) + File.separator;
			} else {
				newCmdText = cmdText.substring(0, cmd.length() + 2)
						+ filePath.substring(pwd.length() + 1) + File.separator;
			}
		}
