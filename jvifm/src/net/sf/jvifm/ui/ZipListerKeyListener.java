...
public class ZipListerKeyListener extends KeyAdapter {
	...
	private void doAction() {
		...
		if (cmd.length() == 1) {
			...
			switch (character) {
			case 't':
				zipLister.tagCurrentItem();
				break;
			case 'v':
				zipLister.switchToVTagMode();
				break;
			// case 'i' : zipLister.switchToOrigMode(); break;

			case 'k':
				zipLister.cursorUp(count);
				break;
			case 'j':
				zipLister.cursorDown(count);
				break;
			case 'l':
				zipLister.enterPath();
				break;
			...
			case 'H':
				zipLister.cursorHead();
				break;
			case 'M':
				zipLister.cursorMiddle();
				break;
			case 'L':
				zipLister.cursorLast();
				break;
			case '^':
				zipLister.cursorTop();
				break;
			case '$':
				zipLister.cursorBottom();
				break;
			case 'G':
				zipLister.cursorBottom();
				break;
