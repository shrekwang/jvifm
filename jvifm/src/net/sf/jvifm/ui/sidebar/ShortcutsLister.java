...
public class ShortcutsLister extends AbstractViLister implements ShortcutsListener {

    //private shortcutsAccessor shortcutsAccess=shortcutsAccessor.getInstance();
    ...
    public void editshortcutsName() {
		...
		newEditor.addKeyListener(new KeyAdapter() {
			public void keyPressed(KeyEvent event) {
				if (event.keyCode == SWT.CR && !newEditor.getText().trim().equals("")) {
    ...
    public void enterPath() {
		...
		if (ext.equals("bat") || ext.equals("sh") || program==null) { //$NON-NLS-1$ //$NON-NLS-2$
			try {
				Runtime.getRuntime().exec(new String[]{file.getPath()},null,file.getParentFile());
			} catch ( Exception e ) {
			}
		} else {
    ...
    public void doDelete() {
    	...
    	for (int i=0; i<selectionIndices.length; i++) {
    		Shortcut shortcuts=(Shortcut)table.getItem(selectionIndices[i]).getData("shortcuts");
