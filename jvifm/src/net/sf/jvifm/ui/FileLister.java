...
public class FileLister implements ViLister,Panel {
   ...
   private static String ENV_OS=System.getProperty("os.name"); //$NON-NLS-1$
    ...
    private void delSelection(int index) {
    	...
    	for (int i=0; i<selections.length; i++){
    		if (index==selections[i]) continue;
    		tmp[i]=selections[i];
    	}
    	table.setSelection(tmp);
    	table.showSelection();
    ...
    private Menu createPopMenu() {
	    ...
	    editItem.addSelectionListener(new SelectionAdapter() {
	    	public void widgetSelected(SelectionEvent arg0) {
	    		...
	    		if (nextEntry == null || file == null ) return;
	    ...
	    openItem.setText(Messages.getString("FileLister.menuitemOpenWithDefault")); //$NON-NLS-1$
    ...
    public void doDelete() {   
    	...
    	String result=new Util().openConfirmWindow(
    			options,Messages.getString("FileLister.warnDialogTitle"),
    			Messages.getString("FileLister.warnDialogMessage"),
    			OptionShell.WARN); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public void doCopyToClipboard() {
		...
		clipboard.dispose();
		fileManager.setTipInfo(selectionFiles.length + " Items copyed to clipboard."); //$NON-NLS-1$
		switchToNormalMode();
    ...
    public void doPaste() {
	    ...
	    if (operate==null || srcFiles == null) return;
    	...
    	public void keyPressed(KeyEvent event) {
    		...
    		if (event.keyCode == SWT.CR ) { //$NON-NLS-1$
				...
				if (action.equals("file")) {
					...
					isSuccess=fileOperator.mkdir(currentFile.getPath());
					  ...
					  isSuccess=currentFile.renameTo( new File(FilenameUtils.concat(pwd,newEditor.getText().trim())));
				}
			...
			if (event.character == SWT.ESC ) {
				if (action.equals("file") || action.equals("folder") ) {
					...
					if (table.getItemCount()>0) table.setSelection(table.getItemCount()-1);
				}
				newEditor.dispose();
				table.setFocus();
			}
		...
		newEditor.addKeyListener(new KeyAdapter() {
			public void keyPressed(KeyEvent event) {
				if (event.keyCode == SWT.CR && !newEditor.getText().trim().equals("")) { //$NON-NLS-1$
				    ...
				    boolean isSuccess=currentFile.renameTo(
				    		new File(FilenameUtils.concat(pwd,newEditor.getText().trim())));
				    if (isSuccess) {
    ...
    public void cursorLast(){
    	...
    	if (currentRow >= table.getItemCount() )  currentRow=table.getItemCount()-1;
    	doSelect();
    }
    public void cursorMiddle(){
    	if (table.getItemCount()<1) return;
    	int count=table.getSize().y / table.getItem(0).getBounds().height-1;
    	if (count>=table.getItemCount()) count=table.getItemCount()-1;
    	currentRow=table.getTopIndex()+(count/2);
    	doSelect();
	}
   ...
   public void setCursorPosition(int row) {
    ...
    public void changePwd(String path) {
    	...
    	if (operateMode!=ORIG_MODE) switchToNormalMode();
		String nextEntry=getItemFullPath(currentRow);
		if (nextEntry != null ) { 
		    File file=new File(nextEntry);
		...
		if (file.isDirectory()) {
			...
			if (pwd.indexOf(File.separator)>-1) {
				int length = pwd.substring(pwd.lastIndexOf(File.separator)).length()-1;
				if (length > 1) {
			...
			lblStatus.setText("total "+currentFiles.length+" items"); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    private String getLastLongestPath(String path) {
    	...
    	if (path.indexOf(File.separator)<0) return path;
    	...
    	while (true) {
    		if (lastMatchIndex<1) break;
    		String tmp=(String)list.get(lastMatchIndex--);
    		
    		if (!tmp.startsWith(longestPath)) break;
    		longestPath=tmp;
    	}
    	if (longestPath.endsWith(File.separator)) longestPath=longestPath.substring(0,longestPath.length()-1);
    	return longestPath;
    ...
    public void addSubFiles(File subFile) {
		...
		TableItem item = new TableItem(table, SWT.BORDER);
		String subFilesPath = subFile.getPath().substring(
				pwd.length() + 1);
    ...
    public void listSubFiles(File[] subFileList) {
    	this.currentFiles=subFileList;
 	    ...
 	    for (int i = 0; i < currentFiles.length; i++) {
 			TableItem item = new TableItem(table, SWT.BORDER);
 			String subFilesPath=currentFiles[i].getPath().substring(pwd.length()+1);
 			item.setText(0, subFilesPath);
 			...
 			item.setText(2, StringUtil.formatDate(currentFiles[i].lastModified()));
 		}
    }
    ...
    private File[] getFilteredFiles(File file,String filterString){
		...
		if (filterString == null) {
			...
			filenameFilter = new AndFileFilter(filenameFilter , new WildcardFilter2(
					filterString));
			subFiles = file.listFiles((FilenameFilter)filenameFilter);
    ...
    private TableItem[] generateItems(File[] subFiles,String selectedName) {
    	 ...
    	 if (subFiles==null || subFiles.length==0) return new TableItem[] {};
    ...
    public void doUnCompress(boolean extractToPwd) {
    	String[] selectionFiles = getSelectionFiles();
    	if (selectionFiles==null || selectionFiles.length<=0) return;
	...
	public void incSearch(String pattern, boolean isForward, boolean isIncrease) {
		...
		if (!isIncrease && !isFind ) fileManager.setTipInfo("file not found"); //$NON-NLS-1$
		redraw();
	}
	...
	public int searchAll(String name) {
		TableItem[] items=table.getItems();
		for (int i=0; i<items.length; i++ ) {
			if (name.equals(items[i].getText(0))) return i;
		}
		return -1;
	}
