...
public class FindCommand extends InterruptableCommand {
	...
	private IOFileFilter findingFilter(CommandLine cmdLine){
		...
		for (int i=0; i<options.length; i++) {
			IOFileFilter filter=createFilter(options[i].getOpt(),options[i].getValue());
	...
	public long parseTime(String timeString) {
		...
		if (timeString.endsWith("m")) {
			return Long.parseLong(timeString.substring(0, timeString.length() - 1)) * 1000*60;
		} else if (timeString.endsWith("h")) {
			return Long.parseLong(timeString.substring(0, timeString.length() - 1)) * 1000*60*60;
		} else if (timeString.endsWith("d")) {
			return Long.parseLong(timeString.substring(0, timeString.length() - 1)) * 1000*60*60*24;
		} else {
			return Long.parseLong(timeString.substring(0, timeString.length() - 1)) * 1000*60*60*24;
		}
	}
