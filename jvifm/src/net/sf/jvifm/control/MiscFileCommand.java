...
public class MiscFileCommand extends Command {
	private static String[] cmdNames = new String[] { "du", "md5sum",
			"sha1sum", "tabnew", "cd", "sort", "swap" };
	...
	public MiscFileCommand(String pwd, String cmdText, String[] args,
	...
	public void execute() {
		...
		if (cmd.equals("sort")) {
			...
			String result = new Util().openConfirmWindow(options, Messages
					.getString("MiscFileCommand.title"), //$NON-NLS-1$ 
			...
			} else if (result.equals(Messages
					.getString("MiscFileCommand.lastModified"))) {
				fileLister.sort("date");
			} else if (result
					.equals(Messages.getString("MiscFileCommand.name"))) {
