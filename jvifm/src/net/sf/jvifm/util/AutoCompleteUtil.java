...
public class AutoCompleteUtil {
	...
	public static List getFileCompleteList(String pwd, String path,boolean onlyDir) {
		...
		for (int i=0; i<files.length; i++) {
    			if (files[i].getName().toLowerCase().startsWith(fileName)
    					...
    					|| FilenameUtils.wildcardMatch(files[i].getName(),fileName,IOCase.INSENSITIVE))
    				if (onlyDir) {
	...
	@SuppressWarnings("unchecked")
	public static String[] getFileCompleteOptions(String pwd, String path,boolean onlyDir) {
		...
		if (bookmarkList!=null ) {
    		for (Iterator it=bookmarkList.iterator(); it.hasNext(); ) {
    			...
    			if (bookmarkName.toLowerCase().startsWith(name)
    					|| FilenameUtils.wildcardMatch(bookmarkName,name,IOCase.INSENSITIVE)) 
    				result.add(bookmark.getPath());
    			
    			/*
    			File bookMarkDir=new File(bookmark.getPath());
    			
    			File[] subFiles= bookMarkDir.listFiles();
    			if (subFiles==null) continue;
    			for (int i=0; i<subFiles.length; i++) {
    				String subFileName=subFiles[i].getName();
	    			if (subFileName.toLowerCase().startsWith(name)
	...
	public static String[] getCommandCompleteList(String cmd) {
		...
		if (customCmdNameList!=null ) {
    		for (Iterator it=customCmdNameList.iterator(); it.hasNext(); ) {
    			...
    			if (cmdName.startsWith(cmd)) result.add(cmdName);
    		}
		}
	...
	public static String[] getShortcutsCompleteList(String cmd) {
		...
		if (customCmdNameList!=null ) {
    		for (Iterator it=customCmdNameList.iterator(); it.hasNext(); ) {
    			String cmdName=(String)it.next();
    			if (cmdName.toLowerCase().startsWith(cmd)) result.add(cmdName);
    		}
		}
		...
		if (customCmdNameList!=null ) {
    		for (Iterator it=customCmdNameList.iterator(); it.hasNext(); ) {
    			...
    			if (cmdName.toLowerCase().startsWith(cmd)
    					|| FilenameUtils.wildcardMatch(cmdName,cmd,IOCase.INSENSITIVE)) 
    				result.add(shortcut);
    		}
		}
